...
func remplacer_espaces_interogation(chaine string) string {
	...
	for i:=0; i<len(chaineR); i++ {
		...
		// S'il y a un espace, on met '?' dans resultat
		if chaineR[i]==' ' {
			resultat[i] = '?'
...
func main() {
	...
	fmt.Printf("Phrase d'origine: %v\nPhrase transformée: %v", chaine, remplacer_espaces_interogation(chaine))
}